...
### 3.1 安全配对与加密 (Security & Encryption)
本设备启用了 **Just Works** 硬件级安全配对。

1.  **配对方式**：无需输入 PIN 码。当手机连接并尝试读写数据时，系统可能会弹出“配对请求”对话框，确认即可。
2.  **自动连接**：支持 **Bonding (绑定)**。第一次配对成功后，后续连接将自动建立加密通道。
3.  **安全性**：所有通讯数据通过 **AES-128 硬件加密**。即使周围有蓝牙嗅探设备，也无法解密你的控制指令。
